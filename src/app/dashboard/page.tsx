      ...
      try {
        ...
        if (produtos) {
          const valorTotal = produtos.reduce(
            (sum: number, p: Produto) =>
              sum + p.preco_venda * p.quantidade_atual,
            0
          )
          const criticos = produtos
            ...
            .sort(
              (a: Produto, b: Produto) =>
                a.quantidade_atual - b.quantidade_atual
            )
            .slice(0, 5)
          setStats((prev) => ({
            ...prev,
  ...
  useEffect(() => {
    if (mostrarBannerPagamento) {
      const timer = setTimeout(
        () => setMostrarBannerPagamento(false),
        8000
      )
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {[
          ...
          <div
            key={label}
            className={`${bgLight} rounded-2xl p-4 md:p-5`}
          >
            <div
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <Link
            key={label}
            href={href}
            className={`${bg} rounded-xl p-4 flex flex-col items-center gap-2 hover:shadow-md transition hover:-translate-y-0.5 text-center`}
          >
      ...
      {/* ══════════ GRID PRINCIPAL (só renderiza se tem dados) ══════════ */}
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-800">
                {produtosVencendo.map((p) => {
                  const diasRestantes = Math.ceil(
                    (new Date(p.data_validade).getTime() -
                      new Date().getTime()) /
                      (1000 * 60 * 60 * 24)
                  )
