...
function movimentoEhVenda(movimento: MovimentoRelatorio): boolean {
  ...
  return (
    motivo.startsWith("PDV-") ||
    motivo.startsWith("PDV -") ||
    motivoBase(motivo) === "venda"
  );
}
  ...
  return (
    <article
      className={`rounded-xl border bg-white p-4 dark:bg-gray-900 ${
        destaque ? paleta.border : "border-gray-200 dark:border-gray-800"
      }`}
    >
      <div
        className={`mb-3 flex h-9 w-9 items-center justify-center rounded-lg ${paleta.bg}`}
      >
...
export default function RelatoriosPage() {
    ...
    async (feedback = false) => {
      ...
      try {
        ...
        if (movimentosRes.error || produtosRes.error) {
          ...
          setErro(
            "Não foi possível carregar todos os dados do relatório. Os resultados anteriores foram mantidos para evitar números incompletos.",
          );
          return;
        }

        const movimentosCarregados =
          (movimentosRes.data as unknown as MovimentoRelatorio[] | null) ?? [];
        const produtosCarregados = (produtosRes.data as Produto[] | null) ?? [];

        setMovimentos(movimentosCarregados);
        setProdutos(produtosCarregados);

        if (movimentosCarregados.length >= 5000) {
          setErro(
            "O período possui muitos registros. O relatório está limitado aos 5.000 movimentos mais recentes.",
          );
        }
  ...
  const vendas = useMemo(
    () => movimentos.filter(movimentoEhVenda),
    [movimentos],
  );
  const perdas = useMemo(
    () => movimentos.filter(movimentoEhPerda),
    [movimentos],
  );
  const entradas = useMemo(
    () => movimentos.filter((item) => item.tipo_movimento === "entrada"),
    [movimentos],
  );
    ...
    () =>
      movimentos.filter(
        (item) =>
          ...
          !movimentoEhVenda(item) &&
          !movimentoEhPerda(item),
      ),
    [movimentos],
  );
  ...
  const relatorioProdutos = useMemo(() => {
    ...
    vendas.forEach((movimento) => {
      const produto =
        movimento.produto ??
        produtos.find((item) => item.id === movimento.produto_id);
    ...
    return Array.from(mapa.values()).sort(
      (a, b) => b.valor_estimado - a.valor_estimado,
    );
  }, [produtos, vendas]);

  const resumoFinanceiro = useMemo(() => {
    const vendasEstimadas = relatorioProdutos.reduce(
      (total, item) => total + item.valor_estimado,
      0,
    );
    const custoEstimado = relatorioProdutos.reduce(
      (total, item) => total + item.custo_estimado,
      0,
    );
    const resultadoEstimado = relatorioProdutos.reduce(
      (total, item) => total + item.resultado_estimado,
      0,
    );
    const produtosComCustoIncompleto = relatorioProdutos.filter(
      (item) => !item.tem_custo,
    ).length;
    const margemEstimada =
      vendasEstimadas > 0 ? (resultadoEstimado / vendasEstimadas) * 100 : 0;
  ...
  const indicadores = useMemo(() => {
    ...
    const itensVendidos = vendas.reduce(
      (total, movimento) =>
        total + Math.max(numero(movimento.quantidade), 0),
      0,
    );
    const itensRecebidos = entradas.reduce(
      (total, movimento) =>
        total + Math.max(numero(movimento.quantidade), 0),
      0,
    );
    const itensPerdidos = perdas.reduce(
      (total, movimento) =>
        total + Math.max(numero(movimento.quantidade), 0),
      0,
    );
    ...
    const produtosZerados = produtosAtivos.filter(
      (produto) => numero(produto.quantidade_atual) <= 0,
    ).length;
  ...
  const produtosSemCusto = useMemo(
    () =>
      produtosAtivos.filter((produto) => numero(produto.preco_custo) <= 0)
        .length,
    [produtosAtivos],
  );
      ...
      <div className="mx-auto max-w-7xl space-y-6 pb-8">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <KPICard
            ...
            valor={indicadores.itensRecebidos}
            icon={ArrowDownLeft}
            cor="emerald"
          />
          <KPICard
            ...
            valor={movimentos
              .filter((movimento) => movimento.tipo_movimento === "saida")
              .reduce((total, movimento) => total + numero(movimento.quantidade), 0)}
            icon={ArrowUpRight}
            cor="gray"
          />
        </div>
        <UpgradeBlock
          titulo="Desbloqueie os relatórios completos"
          descricao="Veja vendas identificadas, perdas, valor do estoque, produtos prioritários, gráficos e exportação CSV."
  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl space-y-6 overflow-x-clip pb-8">
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm font-semibold text-gray-700 disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
            >
              <RefreshCw
            ...
            {temExportarCSV ? (
              ...
              <Link
                href="/assinar"
                className="inline-flex items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 px-4 py-2.5 text-sm font-semibold text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300"
              >
      ...
      <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-xs leading-relaxed text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
        <strong>Leitura responsável:</strong> vendas são apenas saídas identificadas
        como “Venda” ou geradas pelo PDV. Valores de venda, custo e perda são
        estimativas calculadas com os preços atuais dos produtos, pois as
        movimentações antigas não guardam o preço histórico.
      </div>
      ...
      {produtosSemCusto > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
          ...
          <strong>{produtosSemCusto} produto(s)</strong> não possuem preço de custo.
          O valor do estoque e das perdas pode ficar incompleto.
        </div>
      )}
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 md:p-6">
        <div className="mb-5">
          <p className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">RESUMO FINANCEIRO</p>
          <h2 className="mt-1 text-lg font-extrabold text-gray-900 dark:text-white">Resultado estimado do período</h2>
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Calculado somente com as movimentações identificadas como venda.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <KPICard label="Vendas estimadas" valor={formatarMoeda(resumoFinanceiro.vendasEstimadas)} icon={ShoppingCart} cor="blue" descricao="Com os preços de venda atuais" />
          <KPICard label="Custo estimado das vendas" valor={formatarMoeda(resumoFinanceiro.custoEstimado)} icon={Wallet} cor={resumoFinanceiro.produtosComCustoIncompleto > 0 ? "amber" : "gray"} descricao={resumoFinanceiro.produtosComCustoIncompleto > 0 ? `${resumoFinanceiro.produtosComCustoIncompleto} produto(s) com cálculo incompleto` : "Todos os itens possuem custo"} destaque={resumoFinanceiro.produtosComCustoIncompleto > 0} />
          <KPICard label="Resultado estimado" valor={formatarMoeda(resumoFinanceiro.resultadoEstimado)} icon={TrendingUp} cor={resumoFinanceiro.resultadoEstimado >= 0 ? "emerald" : "red"} descricao="Venda estimada menos custo estimado" destaque={resumoFinanceiro.resultadoEstimado < 0} />
          <KPICard label="Margem estimada" valor={`${resumoFinanceiro.margemEstimada.toFixed(1)}%`} icon={BarChart3} cor={resumoFinanceiro.margemEstimada >= 0 ? "emerald" : "red"} descricao="Percentual sobre as vendas estimadas" destaque={resumoFinanceiro.margemEstimada < 0} />
        </div>
        {resumoFinanceiro.produtosComCustoIncompleto > 0 && (
          <div className="mt-4 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-relaxed text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
            <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
            <p>O resultado e a margem podem estar incompletos porque existem produtos vendidos sem preço de custo cadastrado.</p>
          </div>
        )}
      </section>

      {saidasNaoClassificadas.length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-800/50 dark:text-gray-300">
          <strong>{saidasNaoClassificadas.length} saída(s) não classificada(s)</strong> não foram consideradas vendas nem perdas. Isso evita apresentar números enganosos.
        </div>
      )}
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900 md:p-6">
        ...
        {movimentosPorDia.length === 0 ? (
          <p className="py-12 text-center text-sm text-gray-500">Sem movimentações no período.</p>
        ) : (
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={movimentosPorDia}>
              ...
              <Bar dataKey="saidas" name="Saídas totais" fill="#9ca3af" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </section>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900 md:p-6">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="font-bold text-gray-900 dark:text-white">Produtos vendidos pelo PDV ou classificados como venda</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">Valores estimados com os preços atuais.</p>
          </div>
          <button type="button" onClick={exportarVendas} className="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-xs font-semibold text-gray-700 dark:border-gray-700 dark:text-gray-300">
            <Download className="h-4 w-4" />
            Exportar vendas
          </button>
        </div>
        {relatorioProdutos.length === 0 ? (
          <p className="py-12 text-center text-sm text-gray-500">Nenhuma venda identificada no período.</p>
        ) : (
            ...
            <table className="w-full min-w-[760px]">
              <thead>
                <tr className="border-b border-gray-200 text-xs uppercase tracking-wide text-gray-500 dark:border-gray-800">
                  ...
                  <th className="px-3 py-3 text-right">Venda estimada</th>
                  <th className="px-3 py-3 text-right">Custo estimado</th>
                  <th className="px-3 py-3 text-right">Resultado estimado</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {relatorioProdutos.map((item) => (
                  <tr key={item.produto_id} className="text-sm">
                    <td className="px-3 py-3 font-semibold text-gray-900 dark:text-white">{item.produto_nome}</td>
                    <td className="px-3 py-3 text-gray-500 dark:text-gray-400">{item.categoria}</td>
                    <td className="px-3 py-3 text-right">{item.quantidade_vendida}</td>
                    <td className="px-3 py-3 text-right">{formatarMoeda(item.valor_estimado)}</td>
                    <td className="px-3 py-3 text-right">{item.tem_custo ? formatarMoeda(item.custo_estimado) : "Incompleto"}</td>
                    <td className="px-3 py-3 text-right font-semibold text-gray-900 dark:text-white">{item.tem_custo ? formatarMoeda(item.resultado_estimado) : "Incompleto"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="rounded-xl border border-violet-200 bg-violet-50/70 p-5 dark:border-violet-800 dark:bg-violet-900/10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-900 dark:text-white">Raio-X Inteligente</h2>
              <p className="mt-1 text-sm leading-relaxed text-gray-600 dark:text-gray-300">Transforme os indicadores do estoque em prioridades, pontos de atenção e ações recomendadas.</p>
            </div>
          </div>
          <Link href="/dashboard/raio-x" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-700">
            Abrir Raio-X
